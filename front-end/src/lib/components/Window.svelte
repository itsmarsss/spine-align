...
<section class="rounded-xl overflow-hidden relative drop-shadow-md bg-light-gray">
	<div class="drop h-8 w-full absolute top-0 left-0 right-0 flex flex-row bg-white justify-between px-4 items-center">
	...
	<slot/>
	<div class="drop h-8 w-full absolute bottom-0 left-0 right-0 flex flex-row bg-white justify-center px-4 items-center gap-4">
		{#each Array(4) as _, i}
			<button on:click={() => {
				imageIndex = i;
